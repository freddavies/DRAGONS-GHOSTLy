...
import warnings
from scipy.interpolate import CubicSpline, RegularGridInterpolator, RectBivariateSpline
...
from pypeit.core import arc as pypeit_arc
...
class Extractor(object):
    ...
    def new_extract(self, data=None, correct_for_sky=True, use_sky=True,
                    optimal=False, find_crs=True, snoise=0.1, sigma=4.5,
        ...
        print("    Running extraction in "+method+" mode.")
        ...
        # UPDATE: The "scattered light" is probably just dark current. Let's measure it directly?
        # UPDATE2: Let's use the scattered light thing for now, but calibrate with real darks later...
#        if method == "new":
#            data += slight
#            dc = np.median(data[25:75,100:-100][~lacos[1][25:75,100:-100]])
#            data -= dc
#            slight = 0*slight + dc
        if method == "new" and np.mean(slight) > 0:
            print("    Recomputing variance array")
            self.vararray = data + slight + self.rnoise
        ...
        if m_noise.c0 <= 0:
            ...
            m_noise.c1 = 1
        noise_model = lambda x: m_noise.c0 + m_noise.c1 * np.maximum(x,0)
        ...
        if method == "new" and self.arm.arm == "red":
        ...
        # TODO: Add the blue bpm!
        if self.arm.arm == "blue":
            extra_bpm[data < -20] = 1
            self.badpixmask |= extra_bpm.astype(bool) * DQ.fail
        ...
        # UPDATE: Parameters re-tuned. It is possible that they are too aggressive -- double-check this if things look weird.
        if method == "new":
            lacos = lacosmic.lacosmic(data,5.0,sigma,1.25,error=np.sqrt(noise_model(data)),mask=extra_bpm.astype(bool))
            # Consider growing the cosmic ray mask? This would increase the number of masked pixels by A LOT though...
        ...
        flat_data = np.copy(flat.data[0]).astype(np.float64)
        ...
        # Use pypeit skysub to get a bspline profile model for the flat in each order
        # We are throwing away the wavelength dependence to make things 1000x faster
        # Turns out those differences are on the percent level so it's probably ok
        
        print("    Determining flat profile from all orders ", end="")
        ...
        for i in range(nm):
            ...
            flat_array = np.copy(flat_data[xmin:xmax+1,:]).T
            ...
            xdata = np.copy(pixel_array_x.flatten())
            
            # Choose which pixels to use in the fit
            fit_mask = (pixel_array_x.flatten() != 0) & (mask_array.flatten() == 0)
            
            # "Renormalize" assuming small variations in flux with wavelength
            invvar = 1/noise_model(flat_array)
            renorm = np.atleast_2d(np.sum(flat_array*fit_mask.reshape(flat_array.shape),axis=1)).T+1
            flat_array = flat_array/renorm
            invvar = (invvar*(renorm**2)).flatten()
            ydata = np.copy(flat_array.flatten())
        
            # throw away some extra junk
            fit_mask = fit_mask & (invvar > 0)
            ...
            ydata = ydata[isrt]
            invvar = invvar[isrt]
            fit_mask = fit_mask[isrt]
            # pix = pixel_array_y.flatten()[isrt]
            
            
            # flat_models[i] = [None for ii in range(nspl_flat)]
            # for ii in range(nspl_flat):
            #     use_mask = (pix > (ii-0.5)*ny/(2*(nspl_flat-1))) & (pix < (ii+0.5)*ny/(2*(nspl_flat-1)))
            #     use_mask = use_mask | (pix > (2*(nspl_flat-1)-ii-0.5)*ny/(2*(nspl_flat-1))) & (pix < (2*(nspl_flat-1)-ii+0.5)*ny/(2*(nspl_flat-1)))
            #     sset, _, yfit, _, _ = pypeit_fitting.bspline_profile(xdata[use_mask],ydata[use_mask],invvar[use_mask],np.ones(1*len(xdata[use_mask])),
            #                                          ingpm=np.ones_like(xdata[use_mask]).astype(bool),kwargs_bspline={'bkspace':50},
            #                                          kwargs_reject={'groupbadpix':True, 'maxrej': 10},
            #                                          quiet=True)
            #     flat_models[ii] = sset.value

            sset, _, yfit, _, _ = pypeit_fitting.bspline_profile(xdata,ydata,invvar,np.ones(1*len(xdata)),
                                                                 ingpm=fit_mask,kwargs_bspline={'bkspace':50},
                                                                 kwargs_reject={'groupbadpix':True, 'maxrej': 10},
                                                                 quiet=True)
        ...
        # OLD SLOW FLAT PROFILE FITTING METHOD.
...
#
#                po = np.append(i*np.ones_like(pixel_array_x[good_pix]),po)
#                px = np.append(pixel_array_x[good_pix],px)
...
#                        
#            # Don't really see a need to make this a 3d interpolator, since the orders
#            # are always evaluated at specific numbers...
...
#                flat_profile[jj] = RectBivariateSpline(ycen,cens,profs[jj])
        ...
        # Let's try to "correct" the flatfield by using bright sky lines in the science data.
        # This is basically what George did for XQR-30. Might be difficult!
        # The idea is as follows: we measure the flat-normalized profiles of several sky lines,
        # and then fit for a step-function deviation between the IFU2 and IFU1+Sky fibers
        #
        ...
        #  ....................................
        #
        # Actually, in practice, because of the binning the difference is actually a bit more smooth
        # So we can instead just use a bspline as before to measure the relative flat field difference.

        if method == "new":
            # Make this a parameter at some point.
            do_fine_corr = True
            if do_fine_corr:
                print("\n    Computing fine correction on the flatfield from sky lines...", end="")
                pn = np.array([])
                px = np.array([])
                pv = np.array([])
                nlines = 0
                ...
                for i in range(nm):
                    ...
                    pixel_array_norm = np.zeros_like(pixel_array)
                    flat_array = flat_model[i](pixel_array_x.flatten())[0].reshape(pixel_array.shape)
                    
                    skyfit, skymodel = do_pypeit_skysub(i,xmin,xmax,pixel_array,pixel_array_y,pixel_array_x,
                                 flat_model,noise_model,lacos,slight_array,arm=self.arm.arm)
                    ...
                    noise_array = noise_model(pixel_array)
                    renorm = np.sum(pixel_array*(pixel_array_x != 0),axis=1).reshape(ny,1)
                    pixel_array_norm = pixel_array/renorm
                    noise_array /= renorm**2
                    good_pix = pixel_array_x != 0
                    pixel_array_norm[good_pix] /= flat_array[good_pix]
                    noise_array[good_pix] /= flat_array[good_pix]**2
                    
                    nlines += len(amps)

                    for line in range(len(amps)):
                        lo = int(cens[line]-1.0*wids[line])
                        hi = int(cens[line]+1.0*wids[line])
                        renorm = np.sum(pixel_array*(pixel_array_x != 0),axis=1)
                        #pixel_array[lo:hi+1,:] -= 0.5*(pixel_array[lo-1,:]+pixel_array[hi+1,:]) # subtract object on average?
                        px = np.append(pixel_array_x[lo:hi+1,:].flatten()[(pixel_array_x[lo:hi+1].flatten() != 0)],px)
                        pn = np.append(pixel_array_norm[lo:hi+1,:].flatten()[(pixel_array_x[lo:hi+1].flatten() != 0)],pn)
                        pv = np.append((1/noise_array[lo:hi+1,:]).flatten()[(pixel_array_x[lo:hi+1].flatten() != 0)],pv)

                print(f"{nlines} modeled.")
                
                debug_fine_corr = False
                
                if nlines < 8:
                    fine_corr = None
                    print("\n    Insufficient lines detected for fine correction (check vignetting of IFU2)")
                else:
                    sort = np.argsort(px)

                    sset, _, yfit, _, _ = pypeit_fitting.bspline_profile(px[sort],pn[sort],pv[sort],np.ones(1*len(px)),
                                                                 ingpm=np.ones_like(px).astype(bool),kwargs_bspline={'bkspace':220},
                                                                 kwargs_reject={'groupbadpix':True, 'maxrej': 10},
                                                                 quiet=True)
                    
                    fine_corr = sset.value
                    
                    if debug_fine_corr:
                        plt.title(f"fine correction from {nlines} lines")
                        plt.scatter(px,pn,c='k',s=0.1)
                        plt.scatter(px[sort],yfit,c='r',s=0.2)
                        plt.ylim(-0.05,1.5)
                        plt.show()
            else:
                fine_corr = None
        ...
        if method == "new": # don't need to do this for arc/flat
            ...
            for i in good_orders:
                ...
                # Choose which pixels to use in the fit
                fit_mask = (pixel_array_x != 0) & (mask_array == 0)
                #if self.arm.arm == "blue":
                skynorm = np.sum(skyfit*fit_mask,axis=1)
                totnorm = np.sum(pixel_array*(~lacos[1][xmin:xmax+1,:].T)*fit_mask,axis=1)
                fit_mask = fit_mask & np.atleast_2d((totnorm/medfilt(skynorm,11) > 1.4)).T
                fit_mask = fit_mask.flatten()
                
                # "Renormalize" assuming small variations in flux with wavelength
                invvar = 1/noise_model(pixel_array+slight_array)
                skysub = pixel_array-skyfit
                renorm = np.atleast_2d(np.sum(skysub*fit_mask.reshape(pixel_array.shape),axis=1)).T+1
                skysub = skysub/renorm
                invvar = (invvar*(renorm**2)).flatten()
                ydata = np.copy(skysub.flatten())
            
                # throw away some extra junk
                fit_mask = fit_mask & (invvar > 0)
                
                px = np.append(xdata[fit_mask],px)
                py = np.append(ydata[fit_mask],py)
                pv = np.append(invvar[fit_mask],pv)
                pj = np.append(pixel_array_y.flatten()[fit_mask],pj)
            ...
            # The object model varies with wavelength -- let's fit a series of bsplines,
            # and then make an interpolation function between them
            nsplit = nspl_obj # centered on 0, 1/4, 1/2, 3/4, 1, using the fact that the profile is symmetric about the center
            if self.arm.arm == "blue":
                nsplit = 1
            obj_models = [None for ii in range(nsplit)]
            if nsplit > 1:
                for ii in range(nsplit):
                    use_mask = (pix > (ii-0.5)*ny/(2*(nsplit-1))) & (pix < (ii+0.5)*ny/(2*(nsplit-1)))
                    use_mask = use_mask | (pix > (2*(nsplit-1)-ii-0.5)*ny/(2*(nsplit-1))) & (pix < (2*(nsplit-1)-ii+0.5)*ny/(2*(nsplit-1)))
                    sset, _, yfit, _, _ = pypeit_fitting.bspline_profile(xdata[use_mask],ydata[use_mask],invvar[use_mask],np.ones(1*len(xdata[use_mask])),
                                                         ingpm=np.ones_like(xdata[use_mask]).astype(bool),kwargs_bspline={'bkspace':50},
                                                         kwargs_reject={'groupbadpix':True, 'maxrej': 10},
                                                         quiet=True)
                    obj_models[ii] = sset.value
            else:
                sset, _, yfit, _, _ = pypeit_fitting.bspline_profile(xdata,ydata,invvar,np.ones(1*len(xdata)),
                                                                     ingpm=np.ones_like(xdata).astype(bool),kwargs_bspline={'bkspace':50},
                                                                     kwargs_reject={'groupbadpix':True, 'maxrej': 10},
                                                                     quiet=True)
                obj_models[0] = sset.value
            #obj_profile_qa = [np.copy(xdata),np.copy(yfit)]
            
            # cool plot to check how profile looks
            ...
            if debug_obj_profile:
                ...
                objmod = np.zeros((nbins,nbins))
                X = np.linspace(px.min(),px.max(),nbins)
                Y = np.linspace(pj.min(),pj.max(),nbins)
                if nsplit > 1:
                    for ii in range(len(Y)):
                        ind = int(np.round((nsplit-1)-np.abs((Y[ii]-0.5*ny)/(ny/(2*(nsplit-1))))))
                        dif = ((nsplit-1)-np.abs((Y[ii]-0.5*ny)/(ny/(2*(nsplit-1)))))-ind
                        objmod1 = obj_models[ind](X)[0]
                        objmod2 = obj_models[ind+int(np.sign(dif))](X)[0]
                        objmod[:,ii] = objmod1*(1-np.abs(dif))+objmod2*np.abs(dif) #obj_models[ind](X)[0]
                else:
                    for ii in range(len(Y)):
                        objmod[:,ii] = obj_models[0](X)[0]
                
                plt.imshow(hist/norm,origin='lower',cmap='Spectral_r',vmax=0.32,vmin=0)
                plt.contour(hist/norm,origin='lower',colors='k',levels=np.linspace(0,0.35,12),linewidths=1)
                plt.contour(objmod,origin='lower',colors='white',levels=np.linspace(0,0.35,12),linewidths=1)
                plt.show()
                
                for ii in range(nsplit):
                    plt.plot(X,obj_models[ii](X)[0])
                plt.show()
                
                #embed()
                
            # OLD SLOW PROFILE FITTING METHOD
#
#            for i in good_orders:
#
#                xmin, xmax = get_xmin_xmax(i,x_map,nx,ny,profile_y_microns,matrices)
...
#                        pixel_array_norm[mask] = (pixel_array-skyfit)[mask]
#                        pixel_array_norm[mask] /= np.sum((pixel_array-skyfit)[mask & (pixel_array_x > 0)])
...
#            obj_profile = RectBivariateSpline(ycen,cens,profs)
        ...
        for i in range(nm):
            ...
            #Y = pixel_array_y.flatten()
            #X = pixel_array_x.flatten()
            #ind = np.round((nspl_flat-1)-np.abs((Y-0.5*ny)/(ny/(2*(nspl_flat-1))))).astype(int)
            #dif = ((nspl_flat-1)-np.abs((Y-0.5*ny)/(ny/(2*(nspl_flat-1)))))-ind
            #fltmod1 = np.zeros_like(X)
            #fltmod2 = np.zeros_like(X)
            #for ii in range(nsplit):
            #    mask = ind == ii
            #    fltmod1[mask] = flat_models[ii](X[mask])[0]
            #for ii in range(nsplit):
            #    mask = ind+np.sign(dif).astype(int) == ii
            #    fltmod2[mask] = flat_models[ii](X[mask])[0]
            #flat_prof = fltmod1*(1-np.abs(dif))+fltmod2*np.abs(dif)
            #flat_prof = flat_prof.reshape(pixel_array.shape)
            ...
            if do_plot:
                ...
                plt.tight_layout()
                plt.savefig(f"extract_iter1_order{self.arm.m_min+i}.pdf",dpi=250)
                plt.close()

                #plt.show()
                #embed()
                
        print("\n    First iteration complete.",end="")
                
#        if do_fine_corr: # Redo everything from the beginning, but now subtracting the object model from the sky lines to get a better fine correction.
#            print("\n    First iteration complete.", end="")
#            print("\n    Recomputing fine correction on the flatfield from sky lines after object subtraction...", end="")
#            pn = np.array([])
#            px = np.array([])
#            pv = np.array([])
#            nlines = 0
...
#                # Subtract object model from data
#                pixel_array -= 0.5*pixel_array_model
#                
#                skyfit, skymodel = do_pypeit_skysub(i,xmin,xmax,pixel_array,pixel_array_y,pixel_array_x,
#                             flat_model,noise_model,lacos,slight_array,arm=self.arm.arm)
#                             
#                amps,_,cens,wids,_,_,_,_ = pypeit_arc.detect_lines(skymodel(np.arange(ny,dtype=float))[0])
#                
#                keep = (amps > 18000*vignetting) # now that we are subtracting the object, we can use weaker sky lines to improve the model...?
#                keep = keep & (wids < 4.0) # only keep narrow lines
#                keep = keep & (cens > 1000) & (cens < ny-1000) # avoid detector edges where tilts are less accurate
#                amps = amps[keep]
#                cens = cens[keep]
#                wids = wids[keep]
#                
#                noise_array = noise_model(pixel_array+np.abs(pixel_array_model))
#                renorm = np.sum(pixel_array*(pixel_array_x != 0),axis=1).reshape(ny,1)
#                pixel_array_norm = pixel_array/renorm
#                noise_array /= renorm**2
#                good_pix = pixel_array_x != 0
...
#            print(f"{nlines} modeled.")
#            
#            debug_fine_corr = True
#            
#            if nlines < 8:
#                fine_corr = None
#                print("\n    Still insufficient lines detected for fine correction (RIP)")
#            else:
#                sort = np.argsort(px)
#                use = (pn[sort] > 0.4) #& (pn[sort] < 1.4)
#
#                sset, _, yfit, _, _ = pypeit_fitting.bspline_profile(px[sort][use],pn[sort][use],pv[sort][use],np.ones(1*len(px[use])),
#                                                             upper=3,lower=2, #,nord=3,maxiter=50,
#                                                             ingpm=np.ones_like(px[use]).astype(bool),kwargs_bspline={'bkspace':300},
#                                                             kwargs_reject={'groupbadpix':True, 'maxrej': 10},
#                                                             quiet=True)
#                
#                fine_corr = sset.value
...
#                if debug_fine_corr:
#                    plt.title(f"fine correction from {nlines} lines")
#                    plt.errorbar(px,pn,yerr=np.sqrt(1/pv),color='k',fmt='.',markersize=1,lw=0.5)
#                    plt.scatter(px[sort][use],yfit,c='r',s=0.5,zorder=10)
#                    plt.ylim(-0.05,1.5)
#                    plt.show()
#                    embed()
...
def get_slit_profile_edges(profile_y_microns,profile_center,detpix_microns):
    ...
    return np.arange(x_ix[0],x_ix[1]+1)
...
def get_spatial_profiles(nord,bins,nspl,ny,po,px,py,pn):
    ...
    for jj in range(nord):
        ...
        for kk in range(nspl):
            use_mask2 = (po == jj) & (px > bins[0]) & (px < bins[-1]) & (py > (kk-0.5)*ny//(nspl-1)) & (py < (kk+0.5)*ny//(nspl-1))
...
def do_pypeit_skysub(i,xmin,xmax,pixel_array,pixel_array_y,pixel_array_x,flat_profile,noise_model,lacos,slight_array,
                     prof_min=0.25, obj_cut=True, debug=False, fine_corr=None, bkspace=1.2, arm="red"):
    # Experimental sky subtraction stuff
    
    # Compute the flat profile everywhere
    profile = flat_profile[i](pixel_array_x.flatten())[0]
    if fine_corr is not None:
        profile *= fine_corr(pixel_array_x.flatten())[0]
    ...
    if obj_cut:
        if arm == "red":
            mask = mask & (pixel_array_x.flatten() < 50)
        else: # arm == "blue"
            mask = mask & (pixel_array_x.flatten() > -50)

    # Now prepare the data for a bspline fit
    xdata = np.copy(pixel_array_y.flatten())
    ydata = np.copy(pixel_array.flatten())
    ...
    sset, _, yfit, _, _ = pypeit_fitting.bspline_profile(xdata,ydata,invvar,np.ones(1*len(xdata)),
                                                      ingpm=mask,kwargs_bspline={'bkspace':bkspace},nord=3,
                                                      kwargs_reject={'groupbadpix':True, 'maxrej': 10},
                                                      quiet = True)
                                                      
    # We now have the first iteration of sky subtraction, but it is likely biased due to Poisson noise in the pixel array
    # Let's redo the variances to deal with this
    invvar = ((profile.flatten()[isrt]**2)/noise_model(yfit*profile.flatten()[isrt]+slight_array.flatten()[isrt]))
    ...
    # One more round? Or is that too greedy?
    invvar = ((profile.flatten()[isrt]**2)/noise_model(yfit*profile.flatten()[isrt]+slight_array.flatten()[isrt]))
    ...
    # Ok maybe we can do it *one* more time...
    invvar = ((profile.flatten()[isrt]**2)/noise_model(yfit*profile.flatten()[isrt]+slight_array.flatten()[isrt]))
...
def do_pypeit_skysub2(i,xmin,xmax,pixel_array,pixel_array_model,pixel_array_y,pixel_array_x,flat_profile,noise_model,lacos,slight_array,
                     prof_min=0.25, debug=False, fine_corr=None, bkspace=1.2, arm="red"):
    # Experimental sky subtraction stuff
    # This version subtracts off the object profile
    ...
    # Subtract object model
    ydata -= pixel_array_model.flatten()
    ...
    # We now have the first iteration of sky subtraction, but it is likely biased due to Poisson noise in the pixel array
    # Let's redo the variances to deal with this
    invvar = ((profile.flatten()[isrt]**2)/noise_model(yfit*profile.flatten()[isrt]+slight_array.flatten()[isrt]+np.abs(pixel_array_model.flatten()[isrt])))
    ...
    # One more round? Or is that too greedy?
    invvar = ((profile.flatten()[isrt]**2)/noise_model(yfit*profile.flatten()[isrt]+slight_array.flatten()[isrt]+pixel_array_model.flatten()[isrt]))
    ...
    if debug:
        plt.imshow(((~lacos[1][xmin:xmax+1,:].T)*(pixel_array_x != 0)*
                   (pixel_array-pixel_array_model-yfit.reshape(pixel_array.shape))*
                   (np.sqrt(invvar.reshape(pixel_array.shape)))).T,
                    vmin=-5,vmax=5,cmap='RdBu_r',origin='lower')
